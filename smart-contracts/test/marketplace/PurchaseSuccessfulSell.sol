...
contract PurchaseSuccessfulSell is Test {
    ...
    function test() public payable {
        ...
        // Preconditions - Change wallet to 'buyer''s account
        vm.prank(buyer);

        // Test - Purchase the item using account 1
        mkpContract.purchase{value: buyer.balance}(buyer, tokenId);

        // Assertions - Validate the new owner
        address newOwner = mkpContract.ownerOf(tokenId);
        assertEq(newOwner, buyer, "The new owner of the item should be the buyer's account.");
    }
